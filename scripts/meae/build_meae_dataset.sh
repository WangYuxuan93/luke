...
do
  l=${lcs[$i]}
  command="poetry run python luke/cli.py build-wikipedia-pretraining-dataset-for-meae $dir/wikidb/${l}wiki.db ${modelname} ${entityvocab} $dir/gold_dataset/dataset/${l} --sentence-splitter=${l} --include-sentences-without-entities --pool-size 8 --chunk-size 1024"
  echo $command
  $command
done
